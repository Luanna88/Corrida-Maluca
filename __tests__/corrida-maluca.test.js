...
import { corrida, calculoBuffsEDebuffs, resetAliadoEInimigo, prevencaoAtributos,setarAliadoEInimigo } from '../src/corrida-metodo'
...
describe('Testes obrigatorios', () => {
  ...
  it('Deve zerar a velocidade ao resetar debuff quando velocidade <= 0', () => {
  const personagem = {
    nome: 'Teste',
    velocidade: -3,
    drift: -1,
  ...
  const resposta = resetAliadoEInimigo(personagem)

  expect(resposta.velocidade).toBe(0)
  expect(resposta.drift).toBe(0)
  expect(resposta.aceleracao).toBe(0)
  expect(resposta.debuffInimigo).toBe('Desativado')
})

it('Deve manter velocidade em 0 ao resetar buff quando velocidade <= 0', () => {
  const personagem = {
    nome: 'Teste',
    velocidade: 0,
    drift: -5,
  ...
  expect(resposta.drift).toBe(0)
  expect(resposta.aceleracao).toBe(0)
  expect(resposta.buffAliado).toBe('Desativado')
})

it('Não deve zerar velocidade se for positiva no reset de debuff', () => {
  const personagem = {
    nome: 'Teste',
    velocidade: 2,
    drift: 1,
  ...
  expect(resposta.velocidade).toBe(3) 
  expect(resposta.drift).toBe(2)
  expect(resposta.aceleracao).toBe(4)
})

 it('Deve zerar a aceleração quando for menor que 0', () => {
    let personagem = {
      velocidade: 5,
      drift: 3,
      aceleracao: -10
    ...
    const resposta = prevencaoAtributos(personagem)

    expect(resposta.aceleracao).toBe(0)
  })

  it('Não deve alterar a aceleração quando ela for maior ou igual a 0', () => {
    let personagem = {
      velocidade: 5,
      drift: 3,
      aceleracao: 4
  ...
  it('Deve zerar velocidade, drift e aceleracao quando estiverem negativas', () => {
    let personagem = {
      velocidade: -5,
      drift: -2,
      aceleracao: -1
